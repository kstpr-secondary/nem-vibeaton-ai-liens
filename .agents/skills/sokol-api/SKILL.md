...
See the **cgltf-loading** skill for the full cgltf → Sokol bridge pattern.

**Do NOT use direct pointer arithmetic on cgltf buffer data.** Always use `cgltf_accessor_unpack_floats` / `cgltf_accessor_unpack_indices` — they handle sparse accessors, stride, and component-type conversion transparently. Direct offset computation is error-prone and explicitly forbidden in the cgltf-loading skill.

```cpp
// Correct approach: extract via unpack functions, then upload to Sokol
std::vector<float> positions(acc->count * 3);
cgltf_accessor_unpack_floats(acc, positions.data(), positions.size());

sg_buffer vbuf = sg_make_buffer(&(sg_buffer_desc){
    .data = SG_RANGE_REF(positions),
    .label = "gltf-vertices"
});
```
...
```
For the `--slang` flag, use `glsl410` for this project's OpenGL 3.3 Core target (not `glsl430` which requires GL 4.3+ features like compute/SSBOs). See the **sokol-shdc** skill for the full shader compilation workflow.
