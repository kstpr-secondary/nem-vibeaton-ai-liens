---
name: test-author
description: Use when a task or queue entry asks for Catch2 unit tests covering engine math, physics integration, collision primitives (AABB-vs-AABB, ray-vs-AABB), impulse response, quaternion/view-projection composition, ECS component lifecycle, renderer mesh-builder vertex/index counts, frustum-plane math, or pure-function logic in `src/renderer/` or `src/engine/`. Also use for - populating `_coordination/queues/TEST_QUEUE.md`, wiring test files into `renderer_tests` or `engine_tests` Catch2 executables, milestone Validation column requesting testable math/physics. Indirect triggers - "write a quick test for X", "cover the physics step with Catch2", "add a raycast test", "smoke-test the Euler integrator". Do NOT use for rendering correctness (human check), game-layer gameplay (no `game_tests` target), shader validation, integration tests needing live GL context, or cross-workstream planning. Load `engine-specialist`, `renderer-specialist`, or a library SKILL when test targets need domain understanding.
