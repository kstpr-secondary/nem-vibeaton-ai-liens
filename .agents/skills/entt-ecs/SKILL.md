...
Reference for `entt` v3/4.x used in the game-engine workstream (`src/engine/`).
The core registry/view API is compatible across both versions.
...
`entt::handle` wraps an entity + registry reference for an object-like interface. Useful when passing entities across function boundaries without carrying a raw registry pointer.

```cpp
entt::handle h{registry, registry.create()};
h.emplace<Transform>(glm::vec3{0.f}, glm::quat{1,0,0,0}, glm::vec3{1.f});

if (auto* rb = h.try_get<RigidBody>()) {
    rb->linear_velocity += impulse;
}

h.destroy();  // destroys the underlying entity + all its components
```

For game-engine internal code, prefer raw `entt::entity` + registry calls. Handles are mostly useful when game-layer code needs to pass "an entity with its registry" to a helper function.
...
For performance-critical hot paths (e.g., physics integration over thousands of entities), prefer `registry.group<T...>()` which forces contiguous component storage. See `references/entt-component-views.md` for details. For MVP, views are sufficient everywhere.
