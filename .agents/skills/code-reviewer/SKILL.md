---
name: code-reviewer
description: Use when a task in `TASKS.md` carries `Validation: REVIEW` or `SPEC-VALIDATE + REVIEW`, when an entry lands in `_coordination/queues/REVIEW_QUEUE.md`, when a milestone is ready to merge and needs the lightweight pre-merge review (Blueprint §9.2 step 5), or when a human asks "is this obviously broken or risky?" / "any memory bugs?" / "anything dumb in this diff?". The role answers one question — **is the C++ or GLSL in this diff obviously incorrect, unsafe, or wasteful enough to block the merge** — and writes a verdict to `_coordination/<workstream>/REVIEWS/<task-id>.md`. Do NOT use for spec-fidelity checks (`spec-validator`), for writing tests (`test-author`), for visual / behavioral correctness (human check per Blueprint §8.6), for style or refactoring (out of scope this hackathon — speed over elegance, AGENTS.md §1), or for re-architecting code. Domain knowledge is NOT part of this skill — load `renderer-specialist`, `engine-specialist`, `game-developer`, or the relevant library SKILL alongside when the diff uses APIs you must judge in context.
