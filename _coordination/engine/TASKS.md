...
| E-023 | Update `engine_app` for E-M3: WASD+mouse FPS-style camera controller, spawn 50+ collidable entities (validates SC-005 raycast scale), highlight entity under crosshair via raycast, display hit info via `fprintf` | LOW | TODO | — | E-021, E-022 | E-M3 | SEQUENTIAL | SPEC-VALIDATE | files: src/engine/app/main.cpp. Note: app currently set up for E-M4 (24 bodies, no raycast highlight). WASD+mouse camera present but 50+ entities and crosshair raycast missing. |
