...
|---|---|---|---|---|---|---|---|---|---|
| R-001 | Wire `sokol-shdc` CMake custom commands: one `add_custom_command` per shader (`magenta.glsl`, `unlit.glsl`, `lambertian.glsl`, `skybox.glsl`, `line_quad.glsl`, `blinnphong.glsl`) routing to `${CMAKE_BINARY_DIR}/generated/shaders/<name>.glsl.h`; add `VIBEATON_REQUIRE_SOKOL_SHDC=ON` guard; add generated dir to `renderer` PUBLIC include path | HIGH | TODO | — | — | SETUP | BOTTLENECK | SELF-CHECK | files: CMakeLists.txt |
...
| R-003 | Verify `cmake/paths.h.in` exists; verify `CMakeLists.txt` generates `${CMAKE_BINARY_DIR}/generated/paths.h` with `PROJECT_SOURCE_ROOT` and `ASSET_ROOT` macros; confirm `renderer` target exposes generated dir via PUBLIC include so downstream targets inherit it | MED | TODO | — | R-001 | SETUP | PG-SETUP-A | SELF-CHECK | files: cmake/paths.h.in, CMakeLists.txt |
| R-004 | Confirm `stb_image` FetchContent entry exists in `CMakeLists.txt` with pinned version; verify PUBLIC include path propagation to `engine` and `game` targets; confirm `STB_IMAGE_IMPLEMENTATION` is NOT defined in any header file | LOW | TODO | — | R-001 | SETUP | PG-SETUP-A | SELF-CHECK | files: CMakeLists.txt |
...
| R-010 | Implement magenta error pipeline helper (file-private in `renderer.cpp`): `#include <generated/shaders/magenta.glsl.h>`; call `shd_magenta_shader_desc(sg_query_backend())`; `sg_make_pipeline` with pass-through MVP vertex + solid magenta fragment; on failure log fatal; store result; call from `renderer_init()` | HIGH | TODO | — | R-001, R-007 | SETUP | SEQUENTIAL | SELF-CHECK | files: src/renderer/renderer.cpp, shaders/renderer/magenta.glsl |
| R-011 | Create `src/renderer/app/main.cpp` driver stub: `sapp_desc sokol_main()` with empty `init_cb`, `frame_cb`, `cleanup_cb`, `event_cb`; title from RendererConfig default | MED | TODO | — | R-002 | SETUP | SEQUENTIAL | NONE | files: src/renderer/app/main.cpp |
...
| ID | Task | Tier | Status | Owner | Depends_on | Milestone | Parallel_Group | Validation | Notes |
|---|---|---|---|---|---|---|---|---|---|
| R-012 | Implement `renderer_init()` in `renderer.cpp`: define `SOKOL_GLCORE33` compile flag via CMakeLists.txt; call `sg_setup({.context = sapp_sgcontext()})`; store RendererConfig; call magenta pipeline helper (R-010); call `simgui_setup({.sample_count = sapp_sample_count()})` | HIGH | TODO | — | R-007, R-010, R-011 | R-M0 | PG-RM0-A | SELF-CHECK | files: src/renderer/renderer.cpp, CMakeLists.txt |
...
| R-014 | Implement `renderer_run()` in `renderer.cpp`: call `sapp_run(desc)` wiring `init_cb`, `frame_cb`, `cleanup_cb`, `event_cb` to internal renderer callbacks; populate width/height/title from stored config | MED | TODO | — | R-007 | R-M0 | PG-RM0-A | SELF-CHECK | files: src/renderer/renderer.cpp |
...
| R-017 | Build `renderer_app`; verify: window opens to clear color, input events logged, clean exit; run `cmake --build build --target engine_app -DUSE_RENDERER_MOCKS=ON` and confirm exit 0 (mock compiles); **human sign-off required (SC-001)** | MED | TODO | — | R-015, R-016 | R-M0 | SEQUENTIAL | SPEC-VALIDATE + REVIEW | files: (verification only) |
...
| R-053 | Write Catch2 tests in `tests/renderer/test_mesh_builders.cpp`: sphere subdivisions=4 → expected vertex count `(4+1)*(4+1)`, index count `4*4*6`; cube → 24 vertices, 36 indices; all sphere normals unit-length; all cube positions within `[-half_extent, half_extent]` | MED | TODO | — | R-021, R-022 | R-M1 | PG-FINAL-A | SELF-CHECK | files: tests/renderer/test_mesh_builders.cpp |
| R-054 | Write Catch2 tests in `tests/renderer/test_handles.cpp`: `renderer_handle_valid({0})` returns false; `renderer_handle_valid({1})` returns true; enqueue with invalid handle does not increment `draw_count`; zero-length line quad does not increment `line_quad_count` | MED | TODO | — | R-009 | SETUP | PG-FINAL-A | SELF-CHECK | files: tests/renderer/test_handles.cpp |
| R-055 | Update `docs/interfaces/renderer-interface-spec.md` status to `FROZEN — v1.0`; add freeze date; commit + push to `feature/renderer`; announce to engine workstream so engine SpecKit can begin | HIGH | TODO | — | R-027 | R-M1 | SEQUENTIAL | SPEC-VALIDATE + REVIEW | files: docs/interfaces/renderer-interface-spec.md |
