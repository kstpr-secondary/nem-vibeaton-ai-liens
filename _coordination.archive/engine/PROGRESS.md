...
| 2026-05-01 | Crash fix: debug OOB detection modified registry (emplace/remove/get) during entt view iteration → assertion `elem == null` in sparse_set.hpp. Deferred all structural changes to post-iteration pass using emplace_or_replace + to_add/to_remove vectors. Also fixed spawn position distributions: `pos_dist(-40,40)` and `spawner_pos(-35,35)` spawned Y values beyond arena bound ±30 → clamped to ±25 for both. OOB count now zero, app runs clean. Engine tests: 118 assertions / 51 test cases pass. |
